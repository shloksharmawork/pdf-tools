...
export default function PdfToPpt() {
    ...
    const handleConvert = async () => {
        ...
        try {
            const response = await api.post('/pdf-to-ppt', formData, {
                responseType: 'blob',
            ...
            setStatus('completed')
            downloadFile(response.data, `${file.name.replace('.pdf', '')}.pptx`)
        } catch (error) {
            setStatus('error')
            console.error(error)
        }
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] px-4">
            <h1 className="text-3xl font-bold mb-8 text-gray-800">PDF to PowerPoint</h1>

            <div className="w-full max-w-4xl bg-white rounded-xl shadow-lg p-8">
                <UploadBox onFileSelect={handleFileSelect} description="Drop PDF to convert to PowerPoint" />
                        ...
                        <div className="mt-8 max-w-md mx-auto">
                            ...
                            {status === 'idle' && (
                                ...
                                >
                                    ...
                                    <Presentation className="ml-2 w-4 h-4" />
                                </button>
                            )}
                            ...
                            {(status === 'uploading' || status === 'processing') && (
                                <div className="space-y-3">
                                    <p className="text-sm text-gray-500 text-center font-medium">
                                        {status === 'processing' ? 'Creating Slides...' : 'Uploading PDF...'}
                                    </p>
                            ...
                            {status === 'completed' && (
                                <div className="text-center bg-green-50 p-6 rounded-xl border border-green-100">
                                    ...
                                    <p className="text-sm text-green-600 mb-4">Your PowerPoint presentation is ready.</p>
                                    <button
            ...
            <div className="mt-12 max-w-2xl text-center space-y-4 text-gray-500">
                <p>Convert your PDF pages into PowerPoint slides.</p>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    <div className="p-4 bg-gray-50 rounded-lg">
                        <span className="font-semibold text-gray-700 block mb-1">Visual Fidelity</span>
                        Slides look exactly like PDF pages
                    </div>
                    <div className="p-4 bg-gray-50 rounded-lg">
                        <span className="font-semibold text-gray-700 block mb-1">Easy to Present</span>
