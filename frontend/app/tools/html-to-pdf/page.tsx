...
export default function HtmlToPdf() {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] px-4">
            <h1 className="text-3xl font-bold mb-8 text-gray-800">HTML to PDF</h1>

            <div className="w-full max-w-4xl bg-white rounded-xl shadow-lg p-8">
                <UploadBox
                    onFileSelect={handleFileSelect}
                    description="Drop HTML file to convert to PDF"
                    accept={{
                        'text/html': ['.html', '.htm']
                    }}
                />
                        ...
                        <div className="mt-8 max-w-md mx-auto">
                            ...
                            {status === 'idle' && (
                                <button
                                    ...
                                    className="w-full py-3 bg-pink-600 hover:bg-pink-700 text-white rounded-lg font-medium transition-colors flex items-center justify-center shadow-md hover:shadow-lg transform active:scale-95"
                                >
                                    Convert to PDF
                            ...
                            {(status === 'uploading' || status === 'processing') && (
                                <div className="space-y-3">
                                    <p className="text-sm text-gray-500 text-center font-medium">
                                        {status === 'processing' ? 'Converting to PDF...' : 'Uploading Document...'}
                                    </p>
            ...
            <div className="mt-12 max-w-2xl text-center space-y-4 text-gray-500">
                <p>Convert your HTML files (.html, .htm) to PDF format for easy sharing.</p>
            </div>
        </div>
    )
}
