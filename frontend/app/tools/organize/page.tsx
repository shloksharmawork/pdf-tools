...
export default function OrganizePDF() {
    ...
    const handleOrganize = async () => {
        ...
        if (!pageOrder) {
            setErrorMessage("Please enter the new page order.")
            return
        }

        const pagePattern = /^[\d,\s]+$/;
        if (!pagePattern.test(pageOrder)) {
            setErrorMessage("Please enter valid page numbers separated by commas (e.g., 2, 1, 3).")
            return
        }
        ...
        formData.append('pages', pageOrder)
        ...
        try {
            ...
            setStatus('completed')
            downloadFile(response.data, `organized_${file.name}`)
        } catch (error: any) {
            setStatus('error')
            let msg = 'Failed to organize PDF. Please check page numbers.'
    ...
    return (
        <div className="max-w-4xl mx-auto text-center">
            <h1 className="text-3xl font-bold mb-4 text-gray-900">Organize PDF</h1>
            <p className="text-gray-600 mb-8">Sort pages of your PDF file however you like.</p>
