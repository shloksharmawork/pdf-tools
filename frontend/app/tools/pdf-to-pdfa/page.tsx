...
export default function PdfToPdfA() {
    ...
    const handleConvert = async () => {
        ...
        try {
            ...
            setStatus('completed')
            downloadFile(response.data, `${file.name.replace('.pdf', '')}_pdfa.pdf`)
        } catch (error) {
            setStatus('error')
            console.error(error)
        }
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] px-4">
            <h1 className="text-3xl font-bold mb-8 text-gray-800">PDF to PDF/A</h1>

            <div className="w-full max-w-4xl bg-white rounded-xl shadow-lg p-8">
                <UploadBox onFileSelect={handleFileSelect} description="Drop PDF to convert to PDF/A format" />
                        ...
                        <div className="mt-8 max-w-md mx-auto">
                            ...
                            {(status === 'uploading' || status === 'processing') && (
                                <div className="space-y-3">
                                    <p className="text-sm text-gray-500 text-center font-medium">
                                        {status === 'processing' ? 'Converting to PDF/A...' : 'Uploading PDF...'}
                                    </p>
                            ...
                            {status === 'completed' && (
                                <div className="text-center bg-green-50 p-6 rounded-xl border border-green-100">
                                    ...
                                    <p className="text-sm text-green-600 mb-4">Your PDF/A file is ready.</p>
                                    <button
            ...
            <div className="mt-12 max-w-2xl text-center space-y-4 text-gray-500">
                <p>Convert your PDFs to ISO-standardized PDF/A format for long-term archiving.</p>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    <div className="p-4 bg-gray-50 rounded-lg">
                        <span className="font-semibold text-gray-700 block mb-1">ISO Standard</span>
                        Compliant with PDF/A-2b
                    </div>
                    <div className="p-4 bg-gray-50 rounded-lg">
                        <span className="font-semibold text-gray-700 block mb-1">Long-term</span>
                        Preserve your documents forever
                    </div>
                    <div className="p-4 bg-gray-50 rounded-lg">
