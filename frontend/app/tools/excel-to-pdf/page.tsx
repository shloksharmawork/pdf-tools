...
export default function ExcelToPdf() {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] px-4">
            <h1 className="text-3xl font-bold mb-8 text-gray-800">Excel to PDF</h1>

            <div className="w-full max-w-4xl bg-white rounded-xl shadow-lg p-8">
                <UploadBox
                    onFileSelect={handleFileSelect}
                    description="Drop Excel spreadsheet to convert to PDF"
                    accept={{
                        ...
                        <div className="mt-8 max-w-md mx-auto">
                            ...
                            {status === 'idle' && (
                                <button
                                    ...
                                    className="w-full py-3 bg-green-600 hover:bg-green-700 text-white rounded-lg font-medium transition-colors flex items-center justify-center shadow-md hover:shadow-lg transform active:scale-95"
                                >
                                    Convert to PDF
                            ...
                            {(status === 'uploading' || status === 'processing') && (
                                <div className="space-y-3">
                                    <p className="text-sm text-gray-500 text-center font-medium">
                                        {status === 'processing' ? 'Converting to PDF...' : 'Uploading Spreadsheet...'}
                                    </p>
                            ...
                            {status === 'completed' && (
                                <div className="text-center bg-blue-50 p-6 rounded-xl border border-blue-100">
                                    ...
                                    <h3 className="text-lg font-bold text-blue-800 mb-1">Conversion Successful!</h3>
                                    <p className="text-sm text-blue-600 mb-4">Your PDF document is ready.</p>
                                    <button
            ...
            <div className="mt-12 max-w-2xl text-center space-y-4 text-gray-500">
                <p>Convert your Excel spreadsheets (.xls, .xlsx) to PDF format securely.</p>
            </div>
        </div>
    )
}
